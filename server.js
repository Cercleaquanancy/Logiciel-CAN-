...
const server = http.createServer((req, res) => {
  ...
  if (method === 'GET' && url === '/api/serre-participations') {
    pool.query(
      'SELECT id, slot_id, participant_username, creator_username, participation_date, status FROM serre_participations ORDER BY participation_date DESC'
    )
      .then(result => sendJson(res, 200, result.rows))
      .catch(err => {
  ...
  if (method === 'POST' && url === '/api/serre-participations') {
    parseJsonBody(req, (err, body) => {
      ...
      const { slot_id, participant_username, creator_username } = body || {};
      ...
      pool.query(
        'SELECT id FROM serre_participations WHERE slot_id = $1 AND participant_username = $2',
        [slot_id, participant_username]
      )
        .then(checkResult => {
          if (checkResult.rowCount > 0) {
            sendJson(res, 409, { success: false, error: 'already_participating' });
            return;
          }
          ...
          const query = `
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id, slot_id, participant_username, creator_username, participation_date, status
          `;
          const params = [id, slot_id, participant_username, creator_username, 'pending'];
  ...
  if (method === 'GET' && url.startsWith('/api/serre-participations/slot/')) {
    const slotId = decodeURIComponent(url.replace('/api/serre-participations/slot/', ''));
    pool.query(
      'SELECT id, participant_username, participation_date, status FROM serre_participations WHERE slot_id = $1 ORDER BY participation_date ASC',
      [slotId]
    )
      .then(result => sendJson(res, 200, result.rows))
      .catch(err => {
        console.error('Erreur SELECT serre_participations for slot :', err);
        sendText(res, 500, 'Erreur serveur');
      });
    return;
  }
